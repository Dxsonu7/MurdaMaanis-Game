...
public class MurdaMaanis {
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		// Loops till the end of wordlist file and adds to the ArrayList
		while (read.hasNext()) {
		 ...
		 * So, we are creating invalidCount variable to keep track of invalid entries
		 * user makes
		 * and when invalid count is 6 or greater we want to stop the Game
		 * And also we are passing our invalidCount as a parameter to printMurdaMaanis
		 * method, which will make our CLI MurdaMaanis (dead person)
		 */
		int invalidCount = 0;
		while (true) {
			...
			// giving user chance to guess out the complete word, when user's validCount is
			// >= 2
	...
	/**
	 * This method prints dead person or MurdaMaanis for each invalid choice we make
	 * till 5 invalid choices!
	 * We are passing our invalidCount as a parameter to printMurdaMaanis method,
	 * which will make our CLI MurdaMaanis (dead person)
	 * 
	 * @param invalidCount
	 */
	...
	/**
	 * This function prints GameState and will be intended to run after every user
	 * input
	 * Also, it returns boolean which will be used to declare the gameState: stop or
	 * continue
	 * 
	 * @param guessWord: iterating through it and comparing it to Guesses
	 * @param Guesses:   when Guesses equals a char of guessWord
	 *                   we are printing out guessWord. If not then we still keeping
	 *                   it decrypted
	 */
	 ...
	 * This method is special inclusion this version of the game where we want to
	 * start game giving
	 * a chance to user to make a guess which is an entire word, when they make it
	 * to two or more valid letter guesses!
